...
int tasha_get_headsetType(struct snd_soc_codec *codec)
{
	struct tasha_priv *tasha = snd_soc_codec_get_drvdata(codec);

/*
	if (!tasha->mbhc_started) {
		pr_err("%s: mbhc is not ready\n", __func__);
		return 0;
	}
*/
	pr_info("%s: mbhc.current_plug = %u\n", __func__, tasha->mbhc.current_plug);

	return (u32)tasha->mbhc.current_plug;
}
EXPORT_SYMBOL(tasha_get_headsetType);
#endif
/* HTC_AUD_END */
